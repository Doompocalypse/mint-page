...
const VisionaryMint = () => {
  ...
  const nftData = NFT_DATA.visionary;

  useEffect(() => {
    const initializeCandyMachine = async () => {
      if (!process.env.NEXT_PUBLIC_CANDY_MACHINE_VISIONARY) {
      ...
      try {
        const cmId = publicKey(process.env.NEXT_PUBLIC_CANDY_MACHINE_VISIONARY);
  ...
  const handleMint = async () => {
    ...
    try {
      ...
      const PRICE = 1000 * Math.pow(10, 6); // 1000 DMC with 6 decimals
      ...
      if (errorMessage.includes("AccountNotInitialized")) {
        ...
        description = "Insufficient DMC tokens to mint this NFT. You need exactly 1000 DMC tokens.";
        ...
        description = "Transaction failed. Please ensure you have exactly 1000 DMC tokens and try again.";
