...
const VanguardMint = () => {
  ...
  const nftData = NFT_DATA.vanguard;

  useEffect(() => {
    const initializeCandyMachine = async () => {
      if (!process.env.NEXT_PUBLIC_CANDY_MACHINE_VANGUARD) {
      ...
      try {
        const cmId = publicKey(process.env.NEXT_PUBLIC_CANDY_MACHINE_VANGUARD);
  ...
  const handleMint = async () => {
    ...
    try {
      ...
      const PRICE = 100 * Math.pow(10, 6); // 100 DMC with 6 decimals
      ...
      if (errorMessage.includes("AccountNotInitialized")) {
        ...
        description = "Insufficient DMC tokens to mint this NFT. You need exactly 100 DMC tokens.";
        ...
        description = "Transaction failed. Please ensure you have exactly 100 DMC tokens and try again.";
